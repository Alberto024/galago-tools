...
sys.path = [
    p for p in sys.path if not any(sub in p.lower() for sub in ["anaconda3", "miniconda", "mamba"])
]
...
class UpdateNotifier(tk.Toplevel):
    ...
    def __init__(self, parent: Any, current_version: str, latest_version: str):
        ...
        # Add a header
        header = ttk.Label(frame, text="Update Available", font=("TkDefaultFont", 14, "bold"))
        header.pack(pady=(0, 10))

        # Add version information
        version_text = f"Your version: {current_version}\nLatest version: {latest_version}"
        ...
        command_label = ttk.Label(command_frame, text="Run:", font=("TkDefaultFont", 9, "bold"))
        command_label.pack(side=tk.LEFT, padx=(0, 5))

        command_text = ttk.Label(
            command_frame, text="pip install --upgrade galago-tools", font=("Courier", 9)
        )
...
class ToolsManager:
    def __init__(self, app_root: tk.Tk, config: Config) -> None:
        ...
        self.clear_button = ttk.Button(
            self.search_frame, text="Clear Logs", command=self.clear_logs
        )
    ...
    def run_subprocess(
        ...
        if confirm_modal:
            box_result = messagebox.askquestion(
                title="Confirm Tool Restart",
                message=f"Are you sure you want to restart {tool_name}-{tool_type}",
            )
            if box_result == "no":
                return None
        try:
            ...
            if result != 0:
                ...
                self.log_text(
                    f"Port {port} for {tool_name} is already occupied. kill process if you want to use this tool",
                    "warning",
                )
    ...
    def populate_tool_buttons(self) -> None:
            ...
            for t in self.config.workcell_config.tools:
                try:
                    create_tool_frame(
                        self.widgets_frame,
                        t.name,
                        lambda t=t: self.run_subprocess(
                            t.type,
                            t.name,
                            t.port,
                            True,
        ...
        restart_all_button = ttk.Button(
            restart_frame, text="Restart All", command=self.run_all_tools
        )
