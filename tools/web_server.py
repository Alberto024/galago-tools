...
from tools.utils import get_local_ip, get_shell_command
...
def display_startup_message(
    ...
    print(f"{Fore.GREEN}{Style.BRIGHT}{rocket} {robot} GALAGO WEB SERVER STARTED{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}")
    print("")
    print(f"{Fore.YELLOW}{package} Version:{Style.RESET_ALL} {galago_version}")
    if update_available:
        print(f"{Fore.RED}    A new version ({latest_version}) is available!{Style.RESET_ALL}")
        print(
            f"{Fore.YELLOW}    Upgrade using: pip install --upgrade galago-tools{Style.RESET_ALL}"
        )
    print(f"{Fore.YELLOW}{clock} Started:{Style.RESET_ALL} {current_time}")
    print(f"{Fore.YELLOW}{id_icon} Session:{Style.RESET_ALL} {LOG_TIME}")
    print(f"{Fore.YELLOW}{computer} Platform:{Style.RESET_ALL} {os.name}")
    print("")
    ...
    print(f"{Fore.BLUE}{refresh} Starting WebSocket and HTTP servers...{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'-' * 80}{Style.RESET_ALL}")
    print("")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        logger.warning(
                            f"Server not responding at {url} after {max_attempts} attempts: {e}"
                        )
...
async def start_tool(tool_name: str, tool_type: str, port: int) -> bool:
    ...
    try:
        ...
        if log_folder:
            ...
            process = subprocess.Popen(cmd, universal_newlines=True, shell=os.name == "nt")
...
async def send_tool_status(websocket: Optional[Any] = None) -> None:
    ...
    message = {"type": "tool_status", "data": tools_status}
...
async def handle_websocket_message(websocket: Any, data: Dict[str, Any]) -> None:
    ...
    try:
        if action == "start_tool":
            ...
            response = {"type": "api_url", "url": current_url, "success": True}
